...
function Map(props) {
  ...
  useEffect(() => {
    ...
    if (window.google && window.google.maps) {
      ...
      script.src =
        "https://maps.googleapis.com/maps/api/js?key=AIzaSyAppNv_hKiqWizwpVnzD4j8xCH1YY8VqsE&libraries=places";
      script.async = true;
    ...
    if (origin && destination && directionsService && directionsRenderer) {
      ...
      directionsService.route(request, async (result, status) => {
        if (status === "OK") {
          ...
          const pathEncoded = result.routes[0].overview_path
            .map((point) => `${point.lat()},${point.lng()}`)
            .join("|");

          const pathParam = `color:0x0000ff|weight:5|${pathEncoded}`;
          const mapImageUrl = `https://maps.googleapis.com/maps/api/staticmap?key=AIzaSyAppNv_hKiqWizwpVnzD4j8xCH1YY8VqsE&size=600x400&center=${origin}&zoom=auto&path=${encodeURIComponent(pathParam)}`;
